...
func TestRoleHandlerCreateAndAssign(t *testing.T) {
	...
	role.NewHandler(role.NewService(role.NewStore(pool)), org.NewStore(pool), audit.NewRecorder(nil, 1)).Register(mux, connect.WithInterceptors(authn.Interceptor(nil, nil, testsupport.RootToken, nil, nil)))
