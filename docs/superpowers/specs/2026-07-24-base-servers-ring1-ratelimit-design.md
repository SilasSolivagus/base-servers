...
| R2 | Gate A 的**桶** | **per-源IP 桶 + 一个全局桶(source-independent)** 并存;IP 桶 **IPv6 按 /64 前缀、IPv4 按 /32** 分桶 | **[评审 C1:per-IP 单独挡不住 DB 洞]** —— 攻击者一个 IPv6 /64 有 2^64 地址、僵尸网络多 IP,每 IP 少量未知-keyid 仍打 DB。全局桶把**总** GetByKeyID 量封死(与 IP 基数无关),是真正的 DB 洞后盾;per-IP 桶做常态公平。IPv6 /64 分桶让 per-IP 在 v6 下非摆设 |
