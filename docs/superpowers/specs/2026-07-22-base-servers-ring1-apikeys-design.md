...
| K9 | 审计 | 签发/吊销进审计(`apikey.issue`/`apikey.revoke`);**不**对每次成功认证发事件(每请求会淹)。**认证失败的 `apikey.auth` denied 事件推 fast-follow** —— 必须在 `apikey.Verifier` 内发(不能在 authn 拦截器发:authn 已被 audit import,反向 import 成环)+ 需喷洒量控制;v1 认证失败仍返回 `Unauthenticated`,只是暂不落审计 | key 触发的真正 authz 决策已被下游审计;签发/吊销(谁建/杀了凭证)是核心信任轨迹,已覆盖;认证失败遥测次要,避 authn↔audit 环 + 量控后再上 |
...
- **认证失败(fail closed)**:坏 CRC/改一位/未知 keyid/已吊销/已过期 → `Unauthenticated`;**成功认证不发审计洪水**。(`apikey.auth` denied 事件推 fast-follow,见 K9。)
