...
- **不可变**:迁移里 `REVOKE UPDATE, DELETE ON audit_events`(至少对应用角色);应用无改/删 RPC。
...
- **篡改可验**:直接 UPDATE 一条 detail(绕过应用)→ `Verify` 返回 `ok=false` 且 `broken_at_seq` 指向该条。
- **不可变**:应用角色对 `audit_events` 无 UPDATE/DELETE 权限(迁移断言 / 尝试即失败)。
...
- **DB 层不可变 × 单一 owner 角色**:目前全服务以 owner/superuser 角色 `base` 连库,owner/superuser 天然绕过 `REVOKE`,故迁移里的 `REVOKE UPDATE, DELETE ON audit_events FROM PUBLIC` 是防御纵深、当前未真正生效。当前 v1 的不可变由两层保证:(a) 应用层无任何 UPDATE/DELETE 审计查询路径(sqlc 只生成 Insert/Select);(b) 与体系结构无关的哈希链 `Verify`(即便 superuser 或裸 psql 直改也可检出,即 §7 的"篡改可验")。**跟进项**:引入最小权限非-owner 应用角色并把 `DATABASE_URL` 切到该角色,届时 `REVOKE` 才真正对应用角色生效、§7 的"应用角色无 UPDATE/DELETE 权限"断言才可跑;这是跨全部表的部署改动,单列为 Ring 1 后续工作。
