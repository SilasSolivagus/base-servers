...
- **最小权限运行时 DB 角色 —— 已评估,DEFER(2026-07-24 专家复核)**:其原始首要目标(强制审计不可变)已被 `0006` 的 role-independent `BEFORE UPDATE/DELETE/TRUNCATE` 触发器更强地达成。剩下的收益只是对**已被攻陷的 app 进程**做通用爆炸半径收窄 —— 一个防御纵深措施,而其前置条件(app 内 RCE)本就已泄漏 KEK、API-key pepper、Keycloak 凭证。因为 app 以 Postgres **superuser + 表 owner** 连库,**不存在便宜的 GRANT/REVOKE 局部方案**(superuser/owner 绕过权限系统,REVOKE 对其纯属摆设);要拿到任何收益必须全量改动 —— 两个连接身份、migrate-以-owner/serve-以-受限 的双池拆分、带外角色供给、**每张表永久的 GRANT 维护税**、以及测试框架重接。对自托管、单运维、pre-revenue 的 alpha 不划算,应在**首个真多租户生产部署或合规驱动**出现时连同角色一起做。**唯一现在该做的**=文档(非代码):在部署运维手册记一条"生产/多租户部署应以非-superuser `base_app` 运行、迁移带外以 owner 跑",并指向本理由。**不要**上 REVOKE 姿态(对 superuser owner 是假硬化信号)。落地方案种子(将来):`base_app` LOGIN/NOSUPERUSER 经 compose `docker-entrypoint-initdb.d` 供给(密码不进版本化迁移);owner 池跑 migrate + `EnsureActive`,受限池 serve;`ALTER DEFAULT PRIVILEGES FOR ROLE base ... GRANT ... TO base_app`(**须在建表迁移前设 + 一次性补已存在表**)+ 表/序列 GRANT;最大陷阱=`ALTER DEFAULT PRIVILEGES` 非追溯且按授予角色作用域,漏设或漏 sequence grant 会静默到生产才 500。
