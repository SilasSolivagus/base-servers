...
**▶ [See it live](https://silassolivagus.github.io/base-servers/)** &nbsp;·&nbsp; the money-demo, and a "should you use it?" self-check
...
**Production hardening — least-privilege DB role:** the dev/compose stack connects to Postgres as the superuser+owner `base`, which is convenient (it runs its own migrations at startup) but means a compromised app process is a full-DB principal. For a hardened / multi-tenant production deployment, run the service under a **non-superuser `base_app` role** (full DML, but no `DROP`/`ALTER`/`TRUNCATE` and no `UPDATE`/`DELETE` on `audit_events`), with **migrations executed out-of-band as the owner** (a separate connection/step) while the serving process uses the restricted role. Audit-log immutability itself does **not** depend on this — it's enforced by a role-independent append-only trigger regardless of connecting role — so this is defense-in-depth, deliberately deferred for the single-operator alpha (see the audit design doc's deferred-items rationale).
